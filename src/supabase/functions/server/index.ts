...
//     return c.json({
//       success: true,
//       client: clientData, // This is the root object now
//       locations: locations || [],
//       wallet: wallet || null,
  ...
  try {
    ...
    const { data: clientData, error: clientError } = await supabaseAdmin
      ...
      .insert([{
        ...
        email,
        phone: phone || null,
        phone_verified: false, // Assuming OTP verification is done in frontend
        metadata: {},
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      }])
...
app.get('/client/profile', async (c: any) => {
  ...
  try {
    ...
    // Return the data in the structure your frontend expects
    // Based on ClientProfile.tsx, it expects locations and familyMembers at the top level.
    return c.json({
      success: true,
      client: clientData,
      locations: locations || [],
