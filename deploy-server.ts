// CareConnect Server - SQL Migration Version
...
app.post('/auth/signup/client', async (c) => {
  try {
...
app.post('/client/locations', async (c) => {
  ...
  try {
    ...
    if (!type || !address || !city) {
      return c.json({ error: 'Type, address and city are required' }, 400);
    }
...
app.post('/client/family-members', async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401);
  }
  ...
  try {
    const { name, relationship, age, metadata } = await c.req.json();
...
app.put('/client/family-members/:id', async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401);
  }
...
app.delete('/client/family-members/:id', async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401);
  }
...
app.get('/client/family-members', async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) {
    return c.json({ error: 'Unauthorized' }, 401);
  }
